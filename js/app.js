/* SimpleRain app shell: auto host/join, host-owned game state, no chat/media/drawing/mode switching. */
...
const profile = {
  name: localStorage.getItem("simplerain-name") || "Player " + MY_ID.slice(2, 5).toUpperCase(),
  icon: localStorage.getItem("simplerain-icon") || ICONS[parseInt(MY_ID.slice(-2), 36) % ICONS.length],
...
function addPlayer(id, name, peerId, icon, preferredColor) {
  ...
  usedColors.add(color);
  const player = { id, name, color, icon: icon || "Rain" };
...
function renderPlayers() {
  ...
  for (const player of visible) {
    ...
    li.innerHTML = `<span class="swatch" style="background:${player.color}"></span>${hostCrown(player.id)}${esc(player.name)}`;
    list.appendChild(li);
  }
