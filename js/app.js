...
function handleHostMsg(peerId, msg) {
  if (msg.t === "hello") {
    ...
    profiles.set(id, { name: p.name, color: p.color, icon: p.icon });
    net.sendTo(peerId, { t: "profile", id, name: p.name, color: p.color, icon: p.icon });
...
function handleClientMsg(msg) {
  if (msg.t === "welcome") {
    ...
    if (msg.id === MY_ID) {
      ...
      profile.icon = msg.icon;
      myColor = msg.color;
      profile.color = msg.color;
      localStorage.setItem("bp2p-name", msg.name);
      localStorage.setItem("bp2p-icon", msg.icon);
...
function buildColorPicker() {
  ...
  for (const c of COLORS) {
    ...
    btn.className = "swatch-opt" + (c === (profile.color || myColor) ? " selected" : "");
